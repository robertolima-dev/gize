...
/// Regen-safety across a **version upgrade** (WS-RC4 / `MIGRATION.md`): simulates upgrading the
/// `gize` binary and re-running `sync` on a project generated by an older version. After an
/// upgrade a file can differ from the *current* templates for two reasons — the developer
/// hand-edited it, or the generator's output changed between versions (STABILITY.md: the exact
/// generated bytes are not stable). `sync` must treat both the same: report drift and **never
/// overwrite without `--force`**, so upgrading gize never silently destroys code on disk.
#[test]
fn sync_after_a_version_upgrade_preserves_on_disk_code() {
    let root = unique_tmpdir();
    apply(
        &root,
        &scaffold::new_project("shop", true, false, false, Dialect::Postgres, None, TS),
    ...
    // (a) a file the developer hand-edited — must survive verbatim.
    let edited = root.join("src/app/users/service.rs");
    let edited_before = format!(
        "{}\n// custom domain logic — must survive the upgrade\n",
        fs::read_to_string(&edited).unwrap()
    );
    fs::write(&edited, &edited_before).unwrap();

    // (b) a file whose bytes an *older* gize emitted differently from the current template.
    let from_old = root.join("src/app/users/handler.rs");
    let old_before = "// emitted by an older gize — intentionally different bytes\n".to_string();
    fs::write(&from_old, &old_before).unwrap();

    // (c) a file that matches the current template — sync is precise, not wholesale.
    let untouched = root.join("src/app/users/model.rs");
    let untouched_before = fs::read_to_string(&untouched).unwrap();

    // Reconcile against what the *upgraded* gize wants.
    let recon = gize_generator::sync::reconcile(&root, &desired_code_plan(&root)).unwrap();
    let drifted: Vec<String> = recon
        .drift
        .iter()
        .map(|op| op.path.display().to_string())
        .collect();
    assert_eq!(recon.drift.len(), 2, "only the two differing files drift");
    assert!(
        drifted.iter().any(|p| p.ends_with("users/service.rs")),
        "the hand edit is drift"
    );
    assert!(
        drifted.iter().any(|p| p.ends_with("users/handler.rs")),
        "the version-delta file is drift"
    );

    // Without --force the upgrade writes over nothing: every drifted file is byte-identical.
    let applied = gize_generator::sync::apply(&root, &recon, false, false).unwrap();
    assert_eq!(applied.left.len(), 2);
    assert!(
        applied.overwritten.is_empty(),
        "nothing overwritten without --force"
    );
    assert_eq!(
        fs::read_to_string(&edited).unwrap(),
        edited_before,
        "hand edit preserved verbatim across the upgrade"
    );
    assert_eq!(
        fs::read_to_string(&from_old).unwrap(),
        old_before,
        "older-version file preserved verbatim (not silently rewritten)"
    );
    assert_eq!(
        fs::read_to_string(&untouched).unwrap(),
        untouched_before,
        "a file already matching the new template is left untouched"
    );

    // --force is the explicit opt-in to adopt the upgraded templates for drifted files.
    let recon = gize_generator::sync::reconcile(&root, &desired_code_plan(&root)).unwrap();
    let forced = gize_generator::sync::apply(&root, &recon, true, false).unwrap();
    assert_eq!(
        forced.overwritten.len(),
        2,
        "--force adopts the new templates"
    );
    assert!(
        !fs::read_to_string(&from_old)
            .unwrap()
            .contains("older gize"),
        "with --force the file is regenerated to the current template"
    );
}
